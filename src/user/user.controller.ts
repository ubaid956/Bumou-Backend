...
export class UserController {
  ...
  /**
   * Get unified count of all unread items across the application (messages, moments, notifications)
   * @param userId Current user ID
   * @returns Object with all unread counts and total
   */
  @Get('unread-count')
  async getUnreadCount(@GetUser('id') userId: string) {
    return await this.userService.getUnreadCount(userId);
  }
}
